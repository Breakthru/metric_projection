function [M,S,t1,t2,scale,reprojection_error] = fact_OMP_articulated(W,epsilon,n_iter,ind1,ind2) 
% Articulated motion recovery, hinge joint
% Orthographic camera, Metric Projection algorithm
% Projection is done via convex approximation, Case 1 is the only one occurring in practice.
% see journal paper appendix for details
%
...
out=optim_axis_convex(W,epsilon,n_iter,Minit,ind1,ind2);
M = out.Motion;
S = out.Shape;
t1 = out.t1;
t2 = out.t2;
